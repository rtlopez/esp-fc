  ...
  switch (_model.config.input.interpolationMode)
  {
    case INPUT_INTERPOLATION_AUTO:
      _model.state.input.interpolationDelta =
          Utils::clamp(_model.state.input.frameDelta, (uint32_t)4000, (uint32_t)40000) *
          0.000001f; // estimate real interval
      break;
    ...
    default: _model.state.input.interpolationDelta = FRAME_TIME_DEFAULT_US * 0.000001f; break;
  }
...
int16_t FAST_CODE_ATTR Input::getFailsafeValue(uint8_t c)
{
  ...
  switch (ich.fsMode)
  {
    case FAILSAFE_MODE_AUTO: return c == AXIS_THRUST ? PWM_RANGE_MIN : PWM_RANGE_MID;
    case FAILSAFE_MODE_SET: return ich.fsValue;
    case FAILSAFE_MODE_INVALID:
    case FAILSAFE_MODE_HOLD:
    default: return _model.state.input.buffer[c];
  }
}
...
Device::InputDevice* Input::getInputDevice()
{
  ...
  else if (_model.isFeatureActive(FEATURE_RX_PWM))
  {
    const int8_t pins[Device::InputPWM::CHANNELS] = {
        _model.config.pin[PIN_INPUT_PWM_0], _model.config.pin[PIN_INPUT_PWM_1], _model.config.pin[PIN_INPUT_PWM_2],
        _model.config.pin[PIN_INPUT_PWM_3], _model.config.pin[PIN_INPUT_PWM_4], _model.config.pin[PIN_INPUT_PWM_5],
        _model.config.pin[PIN_INPUT_PWM_6], _model.config.pin[PIN_INPUT_PWM_7],
    };

    bool hasPin = false;

    for (size_t i = 0; i < Device::InputPWM::CHANNELS; i++)
    {
      if (pins[i] >= 0)
      {
        hasPin = true;
    ...
    if (!hasPin)
    {
      _model.logger.err().logln(F("RX PWM no pins"));
      return nullptr;
    }

    _pwm.begin(pins, Device::InputPWM::CHANNELS);
