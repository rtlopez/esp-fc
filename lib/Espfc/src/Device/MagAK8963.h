...
class MagAK8963: public MagDevice
{
  public:
    ...
    int begin(BusDevice * bus, uint8_t addr, uint8_t masterAddr) override
    {
      ...
      writeMaster(MPU9250_USER_CTRL, MPU9250_I2C_MST_EN);

      // set the I2C bus speed to 400 kHz
      writeMaster(MPU9250_I2C_MST_CTRL, MPU9250_I2C_MST_CLK);
    ...
    {
      uint8_t buffer;
      if(readMaster(AK8963_WHO_AM_I, 1, &buffer) != 1) return false;
  ...
  private:
    ...
    {
      return _bus->writeByte(_masterAddr, regAddr, data);
    }
    ...
    int8_t readSlave(uint8_t regAddr, uint8_t length, uint8_t *data)
    {
      ...
      if(!_bus->writeByte(_masterAddr, MPU9250_I2C_SLV0_ADDR, _addr | 0x80)) {
        return -1;
      }
      // set the register to the desired AK8963 sub address
      if(!_bus->writeByte(_masterAddr, MPU9250_I2C_SLV0_REG, regAddr)) {
        return -2;
      }
      // enable I2C and request the bytes
      if(!_bus->writeByte(_masterAddr, MPU9250_I2C_SLV0_CTRL, MPU9250_I2C_SLV0_EN | length)) {
        return -3;
      }

      // bit optimized burst version
      //uint8_t out[3] = { _addr | 0x80, regAddr, MPU9250_I2C_SLV0_EN | length };
      ...
      return _bus->read(_masterAddr, MPU9250_EXT_SENS_DATA_00, length, data);
    }

    int8_t writeSlave(uint8_t regAddr, uint8_t data)
    {
      // set slave 0 to the AK8963 and set for write
      if(!_bus->writeByte(_masterAddr, MPU9250_I2C_SLV0_ADDR, _addr)) {
        return -1;
      }
      // set the register to the desired AK8963 sub address 
      if(!_bus->writeByte(_masterAddr, MPU9250_I2C_SLV0_REG, regAddr)) {
      ...
      if(!_bus->writeByte(_masterAddr, MPU9250_I2C_SLV0_DO, data)) {
        return -3;
      }
      // enable I2C and send 1 byte
      if(!_bus->writeByte(_masterAddr, MPU9250_I2C_SLV0_CTRL, MPU9250_I2C_SLV0_EN | (uint8_t)1)) {
        return -4;
      }
