...
int Mixer::begin()
{
  ...
  _model.logger.info().log("MOTOR").log(EscDriver::getProtocolName((EscProtocol)_model.config.output.protocol)).log(_model.config.output.async).log(_model.config.output.rate).log(_model.config.output.dshotTelemetry).logln(ESC_DRIVER_MOTOR_TIMER);
  ...
  if(_model.config.output.servoRate)
  {
    ...
    _model.logger.info().log("SERVO").log(EscDriver::getProtocolName(ESC_PROTOCOL_PWM)).log(true).logln(_model.config.output.servoRate).logln(ESC_DRIVER_SERVO_TIMER);
  }
