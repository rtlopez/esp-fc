...
inline float map3(float x, float in_min, float in_neutral, float in_max, float out_min, float out_neutral,
                  float out_max)
{
  return (x < in_neutral) ? map(x, in_min, in_neutral, out_min, out_neutral)
...
inline void peakDetect(float* samples, size_t begin_bin, size_t end_bin, float bin_width, Peak* peaks,
                       size_t peak_count)
{
